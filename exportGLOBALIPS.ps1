# === Config: Export Global IP Access Rules ===
$headers = @{
...
$outputJsonFile   = "$env:USERPROFILE\Desktop\account_ip_access_rules.json"
...
do {
    $url = "https://api.cloudflare.com/client/v4/accounts/$accountId/firewall/access_rules/rules?page=$page&per_page=$perPage"
...
if ($allRules.Count -gt 0) {
    ...
    Write-Host "Exported $($allRules.Count) global IP rules to JSON: $outputJsonFile"
}
